...
function getIconForSpace(space) {
    const assetPath = getAssetIconPath(space);
    if (assetPath) {
        return `<div class="space-icon"><img src="${assetPath}" alt="" aria-hidden="true"></div>`;
    }

    const iconById = {
        0: `<svg viewBox="0 0 24 24" aria-hidden="true">
                <circle cx="12" cy="12" r="10" fill="#1fb25a"/>
                <path d="M18 12H9l2.8-2.8-1.4-1.4L5 13l5.4 5.2 1.4-1.4L9 14h9z" fill="#fff"/>
            </svg>`,
...
function getAssetIconPath(space) {
    if (space.name === 'Chance') {
        return 'assets/Chance.svg';
    }

    if (space.name === 'Community Chest') {
        return 'assets/CommunityChest.svg';
    }

    if (space.name === 'Free Parking') {
        return 'assets/FreeParking.svg';
    }

    if (space.name === 'Just Visiting / Jail') {
        return 'assets/Jail.svg';
    }

    if (space.type === 'railroad') {
        return 'assets/Railroad.svg';
    }

    if (space.type === 'tax') {
        return 'assets/TaxSpace.svg';
    }

    if (space.name === 'Water Works') {
        return 'assets/waterworks.svg';
    }
...
export function updateBoardOwnership(properties, players) {
    ...
    document.querySelectorAll('.space').forEach(spaceEl => {
        ...
        spaceEl.style.removeProperty('--owner-color');
    });
    ...
    Object.entries(properties).forEach(([spaceId, data]) => {
        ...
        if (spaceEl && owner) {
            spaceEl.style.setProperty('--owner-color', owner.color);
            spaceEl.classList.add('owner-glow');
        }
    });
}

export function renderPropertyImprovements(properties) {
    document.querySelectorAll('.improvement-markers').forEach((el) => el.remove());
    ...
    Object.entries(properties).forEach(([spaceId, data]) => {
        ...
        const count = Math.max(0, Math.trunc(Number(data?.houses) || 0));
        if (count === 0) {
            return;
        }

        const markers = document.createElement('div');
        markers.className = 'improvement-markers';

        if (count >= 5) {
            const hotel = document.createElement('div');
            hotel.className = 'improvement-marker hotel-marker';
            hotel.title = 'Hotel';
            markers.appendChild(hotel);
        } else {
            for (let i = 0; i < count; i += 1) {
                const house = document.createElement('div');
                house.className = 'improvement-marker house-marker';
                house.title = `House ${i + 1}`;
                markers.appendChild(house);
            }
        }

        const spaceEl = document.getElementById(`space-${spaceId}`);
        if (spaceEl) {
            spaceEl.appendChild(markers);
        }
    });
}
...
export function renderHistory(container, history) {
    ...
    [...history].reverse().forEach(item => {
        ...
        div.style.backgroundColor = item.playerColor || 'rgba(255, 255, 255, 0.03)';
        div.style.color = getContrastTextColor(item.playerColor);
...
function getContrastTextColor(backgroundColor) {
    ...
    const color = backgroundColor.trim().replace('#', '');
    if (color.length !== 6) {
        return '';
    }
